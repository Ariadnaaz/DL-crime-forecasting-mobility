...
def calculate_performance_per_threhold(ref_city, crime_agg, seed):
    ...
    with open(f"predictions/rf_i_{ref_city}_{crime_agg}_{seed}.pkl", 'rb') as f:
        subgrid_indices = pickle.load(f)

    with open(f"predictions/rf_predictions_{ref_city}_{crime_agg}_{seed}.pkl", 'rb') as file:
        data_pred = pickle.load(file)

    with open(f"predictions/rf_targets_{ref_city}_{crime_agg}_{seed}.pkl", 'rb') as file:
