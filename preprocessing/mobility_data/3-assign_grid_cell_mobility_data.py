...
Original file is located at
    https://colab.research.google.com/drive/1RBGPE0_Vgc_L0hTfygxPvUaEFZEN8f3Y
"""
...
tts = gTTS('Que te den, tururu!') #Provide the string to convert to speech
...
def make_list_unique_pois(city_selected,input_folder):
  df1 = pd.read_csv(f'drive/MyDrive/PhD_thesis/Mobility_data/preprocessed_data/{input_folder}/Mobility_data_2019_week_01.csv',index_col=0)
  ...
  for year in ['2019','2020','2021','2022','2023']:
    ...
    for week in range(1,53):
      #print("week: ",week)
      df2 = pd.read_csv(f'drive/MyDrive/PhD_thesis/Mobility_data/preprocessed_data/{input_folder}/Mobility_data_{year}_week_{week:02}.csv',index_col=0)
...
def generate_mobility_pois_to_grid_num_map(city_name,city_folder,input_folder,output_folder,grid_size=50): # needs the weekly files of all years
  ...
  with open(f'drive/MyDrive/PhD_thesis/Mobility_data/preprocessed_data/{output_folder}/{city_folder}_mobility_all_grid_map.pkl', 'wb') as fp:
...
"""### 2. Create files with grid cells per year (pre-grids) - old version to only do generall footfall"""

# old code getting the footfall for all poi categories together
def hour_of_week(year,df):
  beginning_of_year = datetime.datetime(year, 1, 1)
  start_week = df['DATE_RANGE_START'].iloc[0]
  end_week = df['DATE_RANGE_END'].iloc[0]
  start = int((start_week - beginning_of_year).total_seconds() // 3600)
  end = int((end_week - beginning_of_year).total_seconds() // 3600)
  return (start,end)

def make_grid_files_year_mobility(city_name,city_folder,path_map,input_folder,output_folder,grid_size=50): # make sure it's correct when using again (had to rewrite bc it didn't save)
  # we assume output folder is inside input folder
  # Read dictionary pkl file
  with open(f'{path_map}/{city_folder}_mobility_all_grid_map.pkl', 'rb') as fp:
  ...
  for year in [2019,2020,2021,2022,2023]:
    ...
    df1 = pd.read_csv(f'drive/MyDrive/PhD_thesis/Mobility_data/preprocessed_data/{input_folder}/Mobility_data_{year}_week_01.csv',index_col=0,parse_dates=['DATE_RANGE_START','DATE_RANGE_END'])
    ...
    for week in range(2,53):
      # load rows for that week for the first week
      df2 = pd.read_csv(f'drive/MyDrive/PhD_thesis/Mobility_data/preprocessed_data/{input_folder}/Mobility_data_{year}_week_{week:02}.csv',index_col=0,parse_dates=['DATE_RANGE_START','DATE_RANGE_END'])
      #df2.columns = map(str.upper, df2.columns) # in case column names are not all caps
    ...
    df_year = df1.copy()
    df_year.to_csv(f"drive/MyDrive/Mobility_data/preprocessed_data/{output_folder}/{city_folder}_mobility_{year}_pre_grid.csv")
    print("Saved file!")

"""### 2. Create files with grid cells per year (pre-grids) - new version for many mobility features"""
...
def make_grid_files_year_mobility_poi_categ(city_name,city_folder,path_map,input_folder,output_folder,grid_size,poi_categ): # make sure it's correct when using again (had to rewrite bc it didn't save)
  # we assume output folder is inside input folder
  # Read dictionary pkl file
  with open(f'drive/MyDrive/PhD_thesis/Mobility_data/preprocessed_data/{path_map}/{city_folder}_mobility_all_grid_map.pkl', 'rb') as fp:
      map_dic = pickle.load(fp)
  map_dic
  ...
  for year in [2019,2020,2021,2022,2023]:
    ...
    df_year = df1.copy()
    df_year.to_csv(f"drive/MyDrive/PhD_thesis/Mobility_data/preprocessed_data/{output_folder}/{city_folder}_mobility_{year}_{poi_categ}_pre_grid.csv")
...
def make_grid_files_year_mobility_poi_diversity(city_name,city_folder,path_map,input_folder,output_folder,grid_size):
  ...
  for year in [2019,2020,2021,2022,2023]:
    ...
    df_year = df1.copy() # why do I copy here? - I guess to make sure I save the final final one??
    #df_year.columns = list(range(1,53))
    print(df_year.shape)
    df_year.to_csv(f"drive/MyDrive/PhD_thesis/Mobility_data/preprocessed_data/{output_folder}/{city_folder}_mobility_{year}_diversity_pre_grid.csv")
    print("Saved file!")
...
def make_final_grid_city_mobility(city_folder,in_out_folder,poi_categ):
  ...
  for year in [2019,2020,2021,2022,2023]:
    print(f"Doing year {year}...")
    df_year = pd.read_csv(f'drive/MyDrive/PhD_thesis/Mobility_data/preprocessed_data/{in_out_folder}/Pre_grid/{city_folder}_mobility_{year}_{poi_categ}_pre_grid.csv',index_col=0)
    ...
    else: # because usinng -0 in the slizing selects zero elements in total
  ...
  df_all.T.to_csv(f"drive/MyDrive/PhD_thesis/Mobility_data/preprocessed_data/{in_out_folder}/Final_all/{city_folder}_mobility_all_{poi_categ}_final_grid.csv")
...
# session crashes bc of using all available RAM :'(
make_final_grid_city_mobility(city_folder='Los_Angeles',
                                in_out_folder='Data_preprocessed_02082024/Grid_cells_0.2gu',
                                poi_categ='diversity')
